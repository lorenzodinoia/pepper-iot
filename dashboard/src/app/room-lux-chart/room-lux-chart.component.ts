...
export class RoomLuxChartComponent implements OnInit {
  ...
  public getfromInterval(start: Date, end: Date): void {
    ...
    this._client.httpClient.get(`${Client.SERVER_URL}/env_data/series/?room_id=${this.id}&field=lux&start=${startAsString}&end=${endAsString}`, Client.OPTIONS).pipe(map((response: any) => {
      return response;
    })).subscribe((response) => {
      let hours: Array<string> = [];
      let values: Array<number> = [];
