...
export class InmateOxygenationChartComponent implements OnInit {
  ...
  public getfromInterval(start: Date, end: Date): void {
    ...
    this._client.httpClient.get(`${Client.SERVER_URL}/vital_signs/series/?inmate_id=${this.id}&field=blood_oxygenation&start=${startAsString}&end=${endAsString}`, Client.OPTIONS).pipe(map((response: any) => {
      return response;
    })).subscribe((response) => {
      let hours: Array<string> = [];
      let values: Array<number> = [];
