...
try:
    ...
    cursor.execute("CREATE TABLE emergency (id INT AUTO_INCREMENT PRIMARY KEY, tmstp datetime NOT NULL, level_em INT, type_em INT, done BOOLEAN, env_data_id INT, vital_signs_id INT, bed_id INT, CONSTRAINT fk_env_emergency FOREIGN KEY (env_data_id) REFERENCES environmental_data(id), CONSTRAINT fk_vital_signs_emergency FOREIGN KEY (vital_signs_id) REFERENCES vital_signs(id), CONSTRAINT fk_bed_emergency FOREIGN KEY (bed_id) REFERENCES bed(id))")
    cursor.execute("CREATE VIEW last_vital_signs AS (SELECT vital_signs.id, tmstp, bpm, body_temperature, min_body_pressure, max_body_pressure, blood_oxygenation, vital_signs.inmate_id from vital_signs WHERE vital_signs.id IN (SELECT MAX(id) FROM vital_signs GROUP BY inmate_id))")
